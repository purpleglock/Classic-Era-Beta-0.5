...
// ⚠️ СЕКТОРА ГЛАВНОГО КАЛИБРА ШИРОКИЕ, И ТРАВЕРЗ ЗАКРЫТ. Было A_END=135°,
// A_SIDE=115°, возвышенные 166° — и ровно на траверзе, на самом естественном
// для боя курсовом угле, ВСЕ четыре главные башни промахивались мимо своих
// секторов на 5-10°. Закрывать щель было нечем: там смотрят спонсоны, а у
// противоминного калибра дальность 186 против 806 у главного (на доске это
// гексы и осмысленно, в непрерывном пространстве арены — нет), и на боевой
// дистанции 400 они не достают. Замер: линкор с ДВЕНАДЦАТЬЮ установками бил
// двумя стволами, а по траверзу — нулём.
// Настоящая башня берёт ~260-280° и слепа только в корму (нос) за надстройкой.
// Отсюда и бортовой залп: на траверзе сходятся и носовые, и кормовые.
const A_END = 4.54,          // 260°: слепа ±50° в противоположную оконечность
      A_SIDE = 3.14;         // 180°: спонсон не бьёт сквозь свой же корпус
const A_SUP = 4.54;          // возвышенная — та же башня, тот же сектор
...
function mountsOf(hull){
  ...
  push( 0.15, 0, 0, A_SUP, 'main');                       // возвышенная носовая
  pair(-0.04, 'sec');                                     // противоминная в миделе
  pair( 0.24, 'aa');                                      // зенитные гнёзда у бака
  push(-0.16, 0, Math.PI, A_SUP, 'main');                 // возвышенная кормовая
