-- ============================================================
-- ЭТАП 3 — УВЕДОМЛЕНИЯ О ПИРАТАХ (прозрачность дохода с караванов)
-- Применять в Supabase → SQL Editor. Идемпотентно.
--
-- ПРОБЛЕМА UX: при атаке пиратов доход с каравана за тик обнулялся МОЛЧА —
--   игрок видел в превью одно, в казне другое, без объяснений.
-- ФИКС: считаем потерю и, если за тик были атаки, кладём ОДНУ сводку в
--   faction_news («Лента сектора»). Логика дохода НЕ изменена — добавлены
--   только подсчёт pirate_loss и запись события.
--
-- ⚠ Это ТОЧНАЯ живая версия economy_accrue (рынок на _res_value) + добавления.
--   Перед применением убедись, что pg_get_functiondef('public.economy_accrue(text)')
--   совпадает по логике (если позже накатывал миграции — пришли свежую версию).
-- ============================================================
...
declare
  ...
  pirate_loss numeric := 0;   -- ← добавлено: оценка потерянного торгового дохода
begin
  ...
  if d >= 1 then
    ...
    for r in select volume, resource, price, convoy, threats, b_fid from public.trade_routes where status='active' and a_fid=p_fid loop
      ...
      if attacked then
        pirate := true;
        -- оценка потери для сводки (что бы отгрузили, не будь нападения)
        ...
        if shipped > 0 then pirate_loss := pirate_loss + shipped * coalesce(r.price,0); end if;
        continue;
      end if;
      avail := coalesce((eco.resources->>r.resource)::numeric,0) + coalesce((res_add->>r.resource)::numeric,0) - coalesce((res_sub->>r.resource)::numeric,0);
      shipped := least(coalesce(r.volume,0)*d, avail);
    ...
    pirate_loss := round(pirate_loss * m_gc);   -- ← в тех же единицах, что и trade_gc
    ...
    -- ── НОВОЕ: сводка об атаке пиратов (одна на тик, в «Ленту сектора») ──
    if pirate then
      insert into public.faction_news(faction_id, faction_name, faction_color, owner_id, owner_email,
        ...
        values (null, '🏴 СВОДКА СЕКТОРА', 'rgba(200,90,80,0.55)', null, null,
          'Караваны ' || coalesce(public._fac_name(p_fid), p_fid) || ' атакованы пиратами',
          null,
          'Торговые конвои фракции ' || coalesce(public._fac_name(p_fid), p_fid) ||
            ' подверглись нападению пиратов на маршруте. '
            || case when pirate_loss > 0
                    then 'Потеряно около ' || round(pirate_loss)::text || ' ГС торгового дохода за период. '
                    else '' end
            || 'Усильте сопровождение конвоев (корабли эскорта снижают риск).',
          'approved', now(), now(), now());
      -- держим не более 20 таких сводок в ленте
      delete from public.faction_news
        where owner_id is null and faction_name = '🏴 СВОДКА СЕКТОРА'
  ...
  return jsonb_build_object('faction_id',eco.faction_id,'gc',eco.gc,'science',eco.science,'agents',eco.agents,
    ...
    'income', jsonb_build_object(
      ...
      'trade',  trade_gc, 'market', market_gc, 'pirate', pirate, 'pirate_loss', pirate_loss));
end$$;
revoke all on function public.economy_accrue(text) from public;

-- ── Проверка ────────────────────────────────────────────────
-- Логика дохода не изменена. После атаки пиратов в «Ленте сектора» (вкладка
-- «Новости») появляется сводка «Караваны <фракция> атакованы пиратами…».
-- В income добавлено поле pirate_loss — клиент может показать сумму потери.
