-- ════════════════════════════════════════════════════════════════════════
--  МИГРАЦИЯ: товарная биржа считает выручку по ПЕРСОНАЛЬНОЙ цене ресурса
--
--  Это полная (живая) economy_accrue с единственным смысловым изменением:
--  в цикле биржи  _res_price(res_rar)  →  _res_value(res_name, res_rar).
--  Проценты по редкости (0.75/0.70/0.65/0.55/0.5) и всё остальное — без изменений.
--  Сортировка «что продать первым» тоже переведена на _res_value (дороже — первым).
--
--  Требует, чтобы _res_value уже существовала (см. _migration_res_value.sql).
--  Применять в Supabase SQL Editor ПОСЛЕ _migration_res_value.sql.
-- ════════════════════════════════════════════════════════════════════════
  ...
  if d >= 1 then
    ...
    if market_cap > 0 then
      ...
      loop
        ...
        -- ЦЕНА теперь персональная (_res_value по имени ресурса), процент — по редкости.
