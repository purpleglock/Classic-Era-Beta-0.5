...
--
-- Суть:
--   1) Храм — НЕ фиксированные 150 ГС/слот, а ретранслятор идей,
--      «кораптящий» население. Ставка за слот считается динамически:
--
--      ставка = clamp(150 × (0.35 + 0.65 × охват^0.7) × рвение × сеть × памятники, 40…240)
...
--
--      Баланс: при полном охвате и всех бонусах потолок 240 ГС/слот
--      (старый флэт был 150); при слабом охвате ставка падает к 52 —
--      «миллиардов из воздуха» у ~200 существующих храмов не будет,
--      наоборот: большим державам придётся строить храмы под население.
--
...
begin
  ...
  return round(least(240, greatest(40,
    150 * (0.35 + 0.65 * power(cov, 0.7)) * fervor * net * monu)));
end$$;
