-- ════════════════════════════════════════════════════════════
-- ОРУЖЕЙНАЯ ВЕРФЬ: боевая цена, очки науки, сид-переполнение
-- ────────────────────────────────────────────────────────────
-- Применять ПОСЛЕ _turret_forge.sql и _turret_forge_units.sql.
--
-- 1) СИД. _tg_norm кастовал сид в int ДО клампа: любой сид > 2^31
--    (генератор их и выдаёт, seed>>>0) валил регистрацию с 22003
--    «value out of range». Кламп теперь до каста, диапазон — uint32.
-- 2) ЦЕНА. gs считался из конструкционного сырья (масса/900 и т.п.) —
--    орудие на 800k урона стоило пару тысяч ГС. Теперь цена боевая:
--    урон^0.86 × дальность × прожорливость. Плюс она БОЛЬШЕ не
--    домножается классовым коэффициентом корпуса в _cn_recompute:
--    сырьё турели обнулено, цена кладётся плоско. Из-за этого число
--    на верстаке и вклад в цену проекта совпадают до единицы.
-- 3) НАУКА. Регистрация орудия разово списывает ОН (stats.on), как
--    публикация проекта техники. Правка сборки добирает разницу.
-- 4) Зеркало клиента: turret_gen.js (TG.stats) и constructors.js.
--    Разошлись формулы — это баг.
--
-- ЦЕНЫ УЖЕ ЗАРЕГИСТРИРОВАННЫХ ОРУДИЙ пересчитываются в §6; цены уже
-- опубликованных проектов пересчитаются при их следующем сохранении.
-- ════════════════════════════════════════════════════════════

-- ── §1. Сид: кламп до каста ──────────────────────────────────
...
begin
  ...
  return coalesce(p_cfg,'{}'::jsonb) || jsonb_build_object(
    ...
    'yaw',     0,
    -- Сид — uint32 (в JS он seed>>>0). Кламп ДО каста: раньше
    -- coalesce(...)::int срабатывал раньше least() и любой большой сид
    -- ронял регистрацию с 22003.
    'seed',    floor(greatest(1, least(4294967295,
                 coalesce(nullif(p_cfg->>'seed','')::numeric, 1337))))::bigint
  );
end$$;
grant execute on function public._tg_norm(jsonb) to authenticated;

-- ── §2. ТТХ: боевая цена + ОН ──────────────────────────────
...
begin
  ...
  -- ЦЕНА ГС — БОЕВАЯ (зеркало TG.stats): раньше считалась из массы через
  -- конструкционное сырьё, и супероружие на 90k урона выходило в 60k ГС,
  -- дешевле корпуса. Теперь платят за мощь: урон — ведущий множитель,
  -- дальность и прожорливость — надбавки. Показатель 0.86 < 1, чтобы
  -- удвоение урона не удваивало цену в лоб.
  ...
  -- ОН — разовая трата науки при регистрации орудия (см. turret_upsert).
  ...
  return jsonb_build_object(
    'ok', true,
    'damage', damage, 'price', price, 'gs', gs, 'on', on_, 'bill', bill,
...
-- ── §3. Резолв орудия дизайна: плоская цена ────────────────
...
begin
  if nullif(w->>'turretId','') is not null then
    ...
    return jsonb_build_object(
      ...
      'crewRequired', 0,
      -- Сырьё турели в конструкционные решения корпуса НЕ идёт: цена своего
      -- орудия плоская (_gs, боевая) и складывается со стоимостью проекта как
      -- есть — ровно то число, что игрок видел на верстаке.
      'resurs',       '{}'::jsonb,
...
-- ── §4. Пересчёт юнита: цена турелей поверх сырья ──────────
...
begin
  ...
  cost := public._cn_kv_cost(kvres, k) + wgs;
...
-- ── §5. Регистрация орудия: разовая трата ОН ─────────────────
-- Зеркало economy_publish_unit: наука списывается с фракции и только с
-- не-стаффа. Правка сборки добирает РАЗНИЦУ (подорожало — доплати,
-- подешевело — ничего не возвращаем: наука уже потрачена).
...
declare
  ...
  charge_fid text; on_cost numeric := 0; old_on numeric := 0;
begin
  ...
  charge_fid := coalesce(p_faction_id, my_fid);
  if not staff and charge_fid is not null then
    on_cost := coalesce((v_st->>'on')::numeric, 0);
    if p_turret_id is not null then
      select coalesce((stats->>'on')::numeric, 0) into old_on
        from public.faction_turrets where id = p_turret_id;
      on_cost := greatest(0, on_cost - coalesce(old_on,0));
    end if;
  end if;
  ...
  if on_cost > 0 then
    update public.faction_economy set science = science - on_cost
      where faction_id = charge_fid and science >= on_cost;
    if not found then raise exception 'не хватает очков науки: нужно % ОН', on_cost; end if;
  end if;
  return row;
end;
$$;
...
-- ── §6. Пересчёт уже зарегистрированных орудий ───────────────
-- Формула цены сменилась — старые строки несут старый gs и не знают про on.
update public.faction_turrets t
   set stats    = public._tg_stats(t.cfg),
       carriers = public._tg_carriers(t.cfg, public._tg_stats(t.cfg))
 where true;
