-- ============================================================
-- ПОЛИТИКА: бонусы исследований + механика «Дом в небесах» (2 системы)
--
-- 1) Политические технологии (pol.*) дают пассивные модификаторы экономики —
--    зеркало EC_POLITICS / EC_RESEARCH_BONUS в economy.js. Применяются в
--    _faction_mods (читается при начислении дохода и расчёте стоимостей).
-- 2) Технология «Дом в небесах» (pol.house_heavens) позволяет захватывать
--    ДВЕ системы за один цикл кулдауна вместо одной.
--
-- Требует: faction_economy.research (jsonb-массив id), current_user_banned().
-- Выполнить целиком в Supabase → SQL Editor. Идемпотентно.
-- ============================================================

-- Счётчик захваченных систем в текущем цикле кулдауна.
alter table public.faction_economy add column if not exists claim_used int default 0;

-- ── 1) Модификаторы доктрины + бонусы политических технологий ──
...
-- ── 2) Захват систем: до 2 за цикл при «Дом в небесах» ──
