-- ============================================================
-- РЕСУРСЫ · ФАЗА 1 (бэкенд): добыча по богатству + режим склад/экспорт + ёмкость
-- Применять в Supabase → SQL Editor ПОСЛЕ _raid_combat.sql (содержит ту же
-- версию economy_accrue + переработку добычи). Идемпотентно.
--
-- Изменения ТОЛЬКО в секции добычи economy_accrue + цены зданий + новая колонка.
-- Фабрики/торговля/рынок/рейды — без изменений. Числа — дефолтные, крутятся.
-- ============================================================

-- ── Множитель богатства месторождения (из amt на карте) ─────
create or replace function public._richness_mult(p_amt text)
returns numeric language sql immutable as $$
  select case btrim(coalesce(p_amt,''))
    when 'колоссально'  then 3.0
    when 'очень много'  then 2.5
    when 'много'        then 2.0
    when 'умеренно'     then 1.5
    when 'мало'         then 1.0
    when 'следы'        then 0.6
    else 1.5 end          -- нет данных о богатстве → среднее
$$;

-- ── Режим добывающего завода: 'store' (на склад) | 'export' ──
alter table public.colony_buildings add column if not exists mine_mode text default 'store';

-- ── Цена нового здания «Склад» (warehouse) ──────────────────
...
-- ════════════════════════════════════════════════════════════
-- economy_accrue — версия из _raid_combat.sql + переработка ДОБЫЧИ.
-- Новое: rate×богатство; режим store/export; export_gc; кап склада.
-- ════════════════════════════════════════════════════════════
...
declare
  ...
  export_gc numeric := 0; cap numeric;     -- ◄ РЕСУРСЫ: доход с экспорта, ёмкость склада
begin
  ...
  if d >= 1 then
    ...
    loop
      for rname in select value from jsonb_array_elements_text(bld.mining_targets) loop
        ...
        if bld.mine_mode = 'export' then
          export_gc := export_gc + rate * d * public._res_value(rname, rr) * 0.6;   -- сразу продаём
        else
    ...
    -- склад: добавляем с КАПОМ ёмкости (сверх лимита — не копится; для этого экспорт)
...
-- ── Проверка ────────────────────────────────────────────────
-- Добыча теперь масштабируется богатством месторождения; завод в режиме 'export'
-- даёт ГС вместо накопления; склад (warehouse) поднимает лимит пула. Клиентский
-- UI (тумблер режима, здание «Склад», шкала ёмкости) — срез 2.
