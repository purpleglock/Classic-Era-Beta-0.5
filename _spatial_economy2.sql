-- ============================================================
-- ПРОСТРАНСТВЕННАЯ ЭКОНОМИКА · СРЕЗ 2 — просперити крутит реальный доход
-- Выполнить ПОСЛЕ _spatial_economy1.sql (нужны _system_balance/_building_vector).
-- Надстройка над economy_accrue из _faith_multi.sql: доход ГС-домиков (фабрика/
-- торг-хаб/храм) умножается на просперити ЕГО системы; фабрика дополнительно
-- получает ценовую премию за товары (price_g) — продаёт в дефицитный рынок дороже.
-- Наука/агенты/добыча/караваны/рынок/экспорт НЕ меняются (это срезы 3+).
-- ⚠ Зеркало в economy.js: ecBuildingIncome/ecGcIncome/ecIncomePreview (срез 2).
-- ============================================================
...
declare
  ...
  sys_prosp jsonb := '{}'::jsonb;                    -- ПРОСТР.ЭК: system_id → просперити
  sys_priceg jsonb := '{}'::jsonb;                   -- ПРОСТР.ЭК: system_id → цена товаров (price_g)
  sb jsonb; v_prosp numeric; v_pg numeric;           -- ПРОСТР.ЭК: рабочие
begin
  ...
  -- ПРОСТР.ЭК (срез 2): просперити и цена товаров по системам фракции (один раз за тик)
  for col in select distinct c.system_id as sid from public.colonies c
    ...
    sb := public._system_balance(col.sid);
    sys_prosp  := jsonb_set(sys_prosp,  array[col.sid], to_jsonb((sb->>'prosperity')::numeric), true);
    sys_priceg := jsonb_set(sys_priceg, array[col.sid], to_jsonb((sb->'prices'->>'g')::numeric), true);
  end loop;

  -- ГС-домики: доход × просперити системы (фабрика ещё × ценовую премию товаров)
  for r in
  ...
  loop
    v_prosp := coalesce((sys_prosp->>r.sid)::numeric, 1);
    if r.btype='factory' then
      v_pg := coalesce((sys_priceg->>r.sid)::numeric, 1);
...
-- select public.economy_accrue('<fid>');   -- income.gc теперь учитывает просперити систем
-- В обзоре «Держава → По системам» полоска просперити; доход фабрик/торг-хабов
-- в дефицитных системах должен просесть, в сбалансированных — вырасти.
