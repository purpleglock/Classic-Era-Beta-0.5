...
// ⚠️ НА ТЕЛЕФОНЕ ПРИБОР УРЕЗАН И ПОДНЯТ. Телеграф, модули и мощность там живут
// под пальцем у краёв экрана (dn_touch.js), и рисовать их ВТОРОЙ раз посреди
// низа — ровно то, из-за чего низ превращался в свалку колец поверх стика и
// кнопки огня. Здесь на пальце остаётся только то, что читают ГЛАЗАМИ:
// живучесть, щит, энергия и сектора обстрела, — и уезжает выше большого пальца.
function hudPanel(x, W, H, s){
  const T = DN.touch;
  if (T){
    const u = clamp(Math.min(W,H)/400, 0.82, 1.45);
    const cx = W/2, cy = H - 272*u, R = 54*u;
