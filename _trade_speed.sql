-- ============================================================
-- ТОРГОВЛЯ · СКОРОСТЬ ФЛОТА → ВРЕМЯ В ПУТИ
-- Применять в Supabase → SQL Editor ПОСЛЕ _trade_multi.sql. Идемпотентно.
--
-- Двигатели торговых кораблей дают скорость. Караван после принятия партнёром
-- сначала ИДЁТ к цели (transit_until), и только по прибытии начинает приносить
-- доход. Быстрый флот доходит за 1 ход, медленный — за несколько. Скорость
-- считает СЕРВЕР из дизайна (мирор скоростей двигателей), клиенту не верим.
-- Зеркало CN_SHIP.engines (constructors.js): speed по классу+индексу двигателя.
-- ============================================================

-- ── Скорость корабля из дизайна (двигатель) ─────────────────
create or replace function public._ship_speed(p_data jsonb)
returns int language sql immutable as $$
  select case p_data->>'class'
    when 'corvette'    then (array[25,40])[coalesce((p_data->>'engine')::int,0)+1]
    when 'frigate'     then (array[23,35])[coalesce((p_data->>'engine')::int,0)+1]
    when 'destroyer'   then (array[19,28])[coalesce((p_data->>'engine')::int,0)+1]
    when 'cruiser'     then (array[20,30])[coalesce((p_data->>'engine')::int,0)+1]
    when 'battleship'  then (array[17,22])[coalesce((p_data->>'engine')::int,0)+1]
    when 'dreadnought' then (array[16,20])[coalesce((p_data->>'engine')::int,0)+1]
    else 20 end
$$;
revoke all on function public._ship_speed(jsonb) from public;
grant execute on function public._ship_speed(jsonb) to authenticated;

-- ── Скорость торгового флота фракции (средняя по грузоподъёмности) ──
create or replace function public._fleet_speed(p_fid text)
returns int language sql stable security definer set search_path=public as $$
  select coalesce(round(
      sum(public._ship_speed(fu.data) * public._ship_cargo(fu.data) * up.qty)::numeric
      / nullif(sum(public._ship_cargo(fu.data) * up.qty), 0)
    )::int, 20)
  from public.unit_production up
  join public.faction_units fu on fu.id = up.unit_id
  where up.faction_id = p_fid and up.category='ship' and up.status='done'
    and public._ship_cargo(fu.data) > 0
$$;
revoke all on function public._fleet_speed(text) from public;
grant execute on function public._fleet_speed(text) to authenticated;

-- ── Время в пути: караван «прибывает» в transit_until ───────
alter table public.trade_routes add column if not exists transit_until timestamptz;

-- ── trade_respond: при принятии считаем время в пути ────────
...
begin
  ...
  -- время в пути: дистанция (смежны origin/dest?) / скорость флота отправителя
  ...
  v_turns := greatest(1, ceil((case when v_adj then 1 else 3 end) * 25.0 / greatest(1, v_speed)));
...
-- ════════════════════════════════════════════════════════════
-- economy_accrue — версия из _trade_multi.sql + пропуск «ещё в пути».
-- ════════════════════════════════════════════════════════════
...
begin
  ...
  if d >= 1 then
    ...
    -- ── КАРАВАНЫ: мультиресурс, поток; в пути (transit_until>now) НЕ приносят ──
...
-- ── Проверка ────────────────────────────────────────────────
-- После принятия караван «в пути» (transit_until) и не приносит доход, пока не
-- прибудет. Быстрый торговый флот (двигатели) доходит за 1 ход, медленный — дольше.
