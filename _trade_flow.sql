-- ============================================================
-- ТОРГОВЛЯ · ПОТОКОВАЯ МОДЕЛЬ: караван возит ПОТОК добычи, а не единицы со склада
-- Применять в Supabase → SQL Editor ПОСЛЕ _diplomacy_trade_coef.sql (та же база accrue).
-- Идемпотентно. Меняется ТОЛЬКО добыча(+mine_flow) и торговая секция economy_accrue.
--
-- Караван экспортирует ресурс, который фракция ДОБЫВАЕТ. За ход уходит
-- throughput = min(грузоподъёмность каравана, добыча этого ресурса). Доход обеих
-- сторон = throughput × цена × dip_coef. Пул ресурсов (склад) караван НЕ дренит.
-- v1: поток каравана не вычитается из режима склад/экспорт добычи (балансный нюанс).
-- ============================================================
...
declare
  ...
  mine_flow jsonb := '{}'::jsonb;     -- ◄ ПОТОК: валовая добыча по ресурсам за период (для караванов)
begin
  ...
  if d >= 1 then
    ...
    loop
      for rname in select value from jsonb_array_elements_text(bld.mining_targets) loop
        ...
        -- валовая добыча ресурса (для караванов-потоков)
    ...
    -- ── КАРАВАНЫ: возят ПОТОК добычи (throughput = min(грузоподъёмность, добыча)) ──
    for r in select volume, resource, price, convoy, threats, b_fid from public.trade_routes where status='active' and a_fid=p_fid loop
      ...
      -- доступно к вывозу = валовая добыча этого ресурса (бюджет делится между караванами)
      avail := coalesce((mine_flow->>r.resource)::numeric, 0);
      shipped := least(coalesce(r.volume,0)*d, avail);   -- volume = грузоподъёмность каравана
...
-- ── Проверка ────────────────────────────────────────────────
-- Караван приносит доход = min(грузоподъёмность, добыча ресурса) × цена × дип-коэф.
-- Склад караваном не дренится; «единиц» в торговле больше нет.
