...
create or replace function public.tech_layout_set(
  ...
  p_icon text default null, p_img text default null)
...
begin
  ...
  insert into public.tech_layout(node_id, x, y, icon, img, updated_at)
    values (p_node, p_x, p_y, nullif(p_icon,''), nullif(p_img,''), now())
...
revoke all on function public.tech_layout_set(text,numeric,numeric,text,text) from public;
grant execute on function public.tech_layout_set(text,numeric,numeric,text,text) to authenticated;
