...
// ⚠️ ЗЕРКАЛО _tg_dict()->'powerBase' (_turret_power_calibration.sql). Числа не
// на глаз: лог-фит по 53 каталожным орудиям с ненулевым power даёт кривые
// E = 1.145·урон^0.716 (корабли), 8.615·урон^0.324 (наземка), 8.877·урон^0.191
// (авиация); база класса = точка этой кривой при уроне эталонного орудия
// класса, платформа — по первому носителю в CLASS_CARRIERS. До калибровки
// шкала верфи расходилась с каталогом в 6-8 раз, и свои орудия жрали столько,
// что реактор кончался раньше слотов.
const POWER_BASE = { drone:14, air:22, apc:36, aa:31, tankgun:81, arty:128,
...
function stats(input){
  ...
  // ЭНЕРГИЯ: база класса × множитель технологии × (урон / урон эталона)^0.716.
  ...
  // Показатель держит «урон на единицу энергии» почти плоским: разгон калибра/
  // стволов не даёт бесплатной эффективности, но и не хоронит крупные орудия.
  // При 0.55 гигантские сборки выходили энергетически ВЫГОДНЕЕ мелких.
  // ⚠️ 0.716 — не подобранное на глаз число, а наклон лог-фита каталожных
  // корабельных орудий (см. POWER_BASE). Наземный фит куда площе (0.32), но
  // брать его нельзя: он делает крупные наземные турели почти бесплатными.
  // Зеркало: _tg_stats в _turret_power_calibration.sql.
  const energy=Math.max(1, Math.round(base*(T.pw||1)*Math.pow(damage/(ref.damage||1),0.716)));
