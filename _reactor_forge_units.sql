-- ════════════════════════════════════════════════════════════
-- РЕАКТОРНАЯ ВЕРФЬ → ПУБЛИКАЦИЯ ЮНИТОВ
-- ────────────────────────────────────────────────────────────
-- Патч ЖИВОГО _cn_recompute: проект теперь несёт data.reactorId, и сервер
-- берёт ТТХ реактора из faction_reactors (_cn_reac_obj), а не из индекса в
-- каталоге. Плюс топливная ведомость: у своей установки закладка посчитана
-- верфью (_fuelBill), каталожная ветка с изотопами остаётся для заводских.
--
-- ⚠ ФАЙЛ СОБРАН ИЗ ДАМПА ЖИВОЙ ФУНКЦИИ (не из репозитория): живой
--   _cn_recompute бывает новее исходников — см. грабли с economy_accrue.
--   Пересобирать так же: node tools/db_dump_fn.js _cn_recompute → патч двух
--   мест (резолв реактора и ведомость) → накат.
-- Порядок применения: ПОСЛЕ _reactor_forge.sql.
-- ════════════════════════════════════════════════════════════

-- Сложение целой ведомости в ведомость: у своей установки позиций несколько
-- (топливо + теплоноситель + защита), и городить цикл внутри огромного
-- _cn_recompute незачем.
create or replace function public._cn_bill_add_many(p_bill jsonb, p_add jsonb)
returns jsonb language plpgsql immutable as $$
declare out jsonb := coalesce(p_bill,'{}'::jsonb); r record;
begin
  for r in select key, value from jsonb_each_text(coalesce(p_add,'{}'::jsonb)) loop
    out := public._cn_bill_add(out, r.key, r.value::numeric);
  end loop;
  return out;
end$$;
grant execute on function public._cn_bill_add_many(jsonb,jsonb) to authenticated;
