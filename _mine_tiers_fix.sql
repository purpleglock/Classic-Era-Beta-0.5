-- ЯРУСЫ ДОБЫЧИ: оживление mining_deep / mining_exotic в живом тике (2026-07-29)
-- Патчит ТОЛЬКО цикл добычи в economy_accrue, снят с живой функции oid 18615.
-- Консолидированный _economy_accrue_consolidated.sql намеренно НЕ применяется.
...
declare
  ...
  mods jsonb; m_mine numeric; m_gc numeric; tier_f numeric;
...
begin
  ...
  if d >= 1 then
    ...
    for bld in
      select cb.btype, cb.mining_targets, coalesce(cb.mine_mode,'store') as mine_mode, c.resources as cres
    ...
    loop
      for rname in select value from jsonb_array_elements_text(bld.mining_targets) loop
        ...
        rate := greatest(1, round(rate * public._richness_mult(relem->>'amt') * m_mine * tier_f));
