-- АВТО-ДОБЫЧА: цикл идёт по залежам планеты, а не по устаревшим mining_targets (2026-07-29)
-- Снято с живой economy_accrue поверх _mine_tiers_fix.sql + _mine_slots_fix.sql.
...
begin
  ...
  if d >= 1 then
    ...
    -- ЯРУСЫ (2026-07-29): до этого цикл брал ТОЛЬКО btype='mining', из-за чего
    -- 1129 построек mining_deep/mining_exotic (2500/8000 ГС каждая) не копали НИЧЕГО.
    -- Редкие ресурсы почти все лежат под этими ярусами → приход rare/epic/legendary
    -- был около нуля (Памятник веры = 600 Реликтового дерева копился 200+ дней,
    -- у Соридонского Союза приход дерева был ровно 0 при наличии залежей).
    -- Теперь копают все три типа. Профильный ярус — полный темп, чужой — 0.8.
    -- Базовый 'mining' штрафа НЕ получает: иначе просели бы фракции без глубоких
    -- шахт (Цветущий Пояс, Галактическая Сеть). Просадка невозможна: цикл только
    -- ДОБАВЛЯЕТ строки к прежнему поведению.
    for bld in
      select cb.btype, coalesce(cb.slots_open,1) as slots_open, coalesce(cb.mine_mode,'store') as mine_mode, c.resources as cres
    ...
    loop
      -- АВТО-ДОБЫЧА (2026-07-29): идём по залежам ПЛАНЕТЫ, а не по mining_targets.
      -- targets заполняются триггером _mining_autofill_targets только при ВСТАВКЕ
      -- постройки, поэтому залежи, появившиеся на колонии позже (геологоразведка,
      -- захват), не копались никогда: 233 шахты не видели 901 залежь. Ручной выбор
      -- всё равно отключён (mining_assign отозван у authenticated), так что список
      -- targets — чистый источник рассинхрона. Больше ни на что не влияет.
      for relem in select value from jsonb_array_elements(bld.cres) loop
        rname := relem->>'name';
        if rname is null then continue; end if;
        -- у старых снимков колоний поле r пустое → добираем редкость из каталога,
        -- иначе легендарная залежь сошла бы за common и попала не на тот ярус
        ...
        tier_f := case when bld.btype = 'mining' then 1.0
                       when public._mine_tier_ok(bld.btype, rr) then 1.0
                       else 0.8 end;
        -- СЛОТЫ (2026-07-29): до этого slots_open в добыче не участвовал вообще —
        -- шахта на 6 слотов копала как на 1, при средней заполненности 5.7/6.
        -- Расширения покупали впустую. БОНУС, а не делитель: 1 слот → ×1.0, 6 → ×1.5.
        -- Делитель (слоты/3 и т.п.) просаживал бы фракции с малыми шахтами.
        slot_f := 1 + 0.10 * (greatest(1, coalesce(bld.slots_open,1)) - 1);
        rate := greatest(1, round(rate * public._richness_mult(relem->>'amt') * m_mine * tier_f * slot_f));
