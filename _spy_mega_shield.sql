-- ════════════════════════════════════════════════════════════
-- МЕГАСООРУЖЕНИЯ НЕ СНОСЯТСЯ АГЕНТАМИ
--
-- Диверсия (`sabotage`) и массовый снос (`mass_demolish`) выбирали жертву
-- как `order by random() limit 1` по ВСЕМ постройкам державы. В эту лотерею
-- попадало и «Ожерелье Немезиды» — 6 000 000 ГС, трое суток стройки, одно
-- на систему: один бросок кубика у горстки агентов стирал мегасооружение.
-- Тем же грешил легаси-RPC `spy_mission(p_type='sabotage')` (клиент его не
-- зовёт, но он открыт наружу и тоже бил по случайной постройке).
--
-- Теперь мегасооружения агентам НЕ ВИДНЫ как цель: выборка их исключает.
-- Если у цели не осталось НИЧЕГО, кроме мегасооружений, операция считается
-- проваленной (`ok=false`, note), а не «съедает» мегаструктуру.
--
-- Единая точка правды — `public._btype_mega(bt)`. Добавить новое
-- мегасооружение = дописать строку в её список.
--
-- ⚠ `_spy_resolve` — внутренняя (без гейта `_fm_gates`), `_fm_wrap` звать НЕ
--   надо. Перекрываем ТОЛЬКО `spy_mission__raw`, обёртку `spy_mission` с
--   гейтом 'spy' не трогаем — иначе гейт слетит.
-- Идемпотентно. Базис — живой prosrc из БД на 13.08.
-- ?v=20260813mega1
-- ════════════════════════════════════════════════════════════

-- ── 1. СПРАВОЧНИК: что считается мегасооружением ───────────
create or replace function public._btype_mega(p_btype text)
returns boolean language sql immutable as $$
  select coalesce(p_btype, '') in (
    'nemesis'        -- ⭘ Ожерелье Немезиды: системная ПРО, 6 000 000 ГС, одно на систему
  )
$$;

comment on function public._btype_mega(text) is
  'Постройка-мегасооружение: недоступна для сноса шпионскими операциями (sabotage/mass_demolish).';
  ...
  for m in select * from public.spy_missions where actor_fid=p_fid and status='active' and ready_at<=now() loop
    ...
    if ok then
      if m.op in ('recon_basic','recon_deep') then
        ...
        -- ⛨ мегасооружения агентам не по зубам
        bid := null; bt := null;
        if m.target_colony is not null then
          select id,btype into bid,bt from public.colony_buildings
            where colony_id=m.target_colony and faction_id=m.target_fid
              and not public._btype_mega(btype)
            order by random() limit 1;
        end if;
        if bid is null then
          select id,btype into bid,bt from public.colony_buildings
            where faction_id=m.target_fid and not public._btype_mega(btype)
            order by random() limit 1;
        end if;
        ...
        else ok := false; res := jsonb_build_object('building',null,'note','no destructible buildings'); end if;
        ...
        if (res->>'count')::int = 0 then
          ok := false; res := res || jsonb_build_object('note','no destructible buildings');
        end if;
...
-- ── 3. ЛЕГАСИ-RPC: та же защита ────────────────────────────
-- Клиент его не зовёт, но функция открыта наружу и била по случайной постройке.
CREATE OR REPLACE FUNCTION public.spy_mission__raw(p_target_fid text, p_type text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
  ...
  if p_type='recon' then
    ...
    if suc then
      if random() < 0.5 then
        ...
        -- ⛨ мегасооружения агентам не по зубам
        select id, btype into bid, bt from public.colony_buildings
          where faction_id=p_target_fid and not public._btype_mega(btype)
          order by random() limit 1;
...
grant execute on function public._btype_mega(text) to anon, authenticated, service_role;
