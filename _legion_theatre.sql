-- ════════════════════════════════════════════════════════════
-- ☠ ТЕАТР ЛЕГИОНА — данные для отдельного режима карты галактики.
-- 17.08. Задача: «выключить границы и всю остальную информацию и ИНТЕРАКТИВНО
-- показать ситуацию с пиратством и их потенциальные ходы и манёвры».
--
-- ⚠ ГЛАВНОЕ ПРАВИЛО СЛОЯ: режим не всеведущ. Он показывает ровно то, что
-- держава ЗНАЕТ через свои глаза (_legion_grade: resolved / signature / ghost),
-- иначе карта превращается в чит-панель бота, и вся сеть разведзастав теряет
-- смысл. Полный маршрут отдаём только по resolved; по signature — текущий
-- отрезок; ghost вообще без точки, одним неспокойным сектором.
--
-- ЦЕПОЧКА: после _legion_contacts.sql, _legion_lair.sql, _legion_bg_starve.sql.
-- Идемпотентно.
-- ════════════════════════════════════════════════════════════
...
grant execute on function public.legion_theatre() to authenticated;
