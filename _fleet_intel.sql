...
--
-- Правила, заданные дизайном:
--   • ФЛОТЫ видят ВСЕ (idle и в полёте) — соединение на карте не спрячешь.
--     Но ЧИСЛЕННОСТЬ/СОСТАВ виден только при разведке цели (basic+ recon)
--     или пассивной развед-связи; иначе на карте «⚓?».
...
returns boolean language sql stable as $$
  select p_viewer = p_owner
      or (public._spy_intel(p_viewer, p_owner)->>'level') is not null;
$$;

-- ── 2. fleets_visible — ВСЕ флоты на карте для текущего игрока ──
-- Возвращает каждый флот (idle и transit) любой державы. mine — мой ли;
-- intel — известен ли состав; ships/composition обнуляются, если состав не вскрыт.
