-- ════════════════════════════════════════════════════════════
-- ОБЛИК ВИДЕН ВРАГУ: pk.gd И pk.dim ИДУТ ОБЕИМ СТОРОНАМ
-- ────────────────────────────────────────────────────────────
-- ПОРЯДОК: после _angel_host.sql. Надмножество `battle_state`.
--
-- ЧТО БЫЛО НЕ ТАК. Метка облика бортов Престола лежит в `pk` (`gd`: 1 —
-- Херувим, 2 — Офаним), и по ней клиент выбирает процедурный спрайт вместо
-- каталожного корпуса. А `battle_state` отдавал `pk` ТОЛЬКО своей стороне —
-- значит игрок, который с ними дерётся, метку не получал вовсе и видел
-- обычные корабли. Тем же местом резалась и ступень свечения ковчега (`dim`):
-- единственная подсказка «оно горит тише, чем в прошлый раз» доставалась
-- только самому ангелу, то есть тому, кому она не нужна.
--
-- ⚠️ ЭТО НЕ ОСЛАБЛЕНИЕ СЕКРЕТНОСТИ. Наружу уходят ровно два ключа, и оба —
-- про внешний вид: ими нельзя посчитать ни печати, ни перезарядки. Всё
-- остальное содержимое pk (перехват `hj`, счётчик попаданий `ang`, разгон
-- «Бимрайдера» `ride`, служебный `seal`) по-прежнему видит только своя сторона.
-- ════════════════════════════════════════════════════════════
