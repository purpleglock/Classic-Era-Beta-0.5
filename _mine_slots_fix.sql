-- СЛОТЫ В ДОБЫЧЕ: slots_open наконец влияет на темп (2026-07-29)
-- Снято с живой economy_accrue поверх _mine_tiers_fix.sql. Бонус 1+0.10*(слоты-1).
...
begin
  ...
  if d >= 1 then
    ...
    for bld in
      select cb.btype, coalesce(cb.slots_open,1) as slots_open, cb.mining_targets, coalesce(cb.mine_mode,'store') as mine_mode, c.resources as cres
