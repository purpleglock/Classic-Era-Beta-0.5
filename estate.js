...
const EST_GL = {
  // Кирка: черенок и ГОЛОВА обязаны сходиться в одной точке. Старый глиф рисовал
  // дугу поперёк всей клетки, черенок обрывался под ней, а сверху болтался
  // вертикальный огрызок — читалось как галочка под зонтом. Теперь голова —
  // дуга ПОПЕРЁК черенка (⊥ к нему), черенок чуть протыкает её в проушине.
  pick:    '<path d="M4.4 19.6 16 8"/><path d="M10.1 6.1Q17.2 6.8 17.9 13.9"/>',
  // Штаб артиллерии («протокол последнего довода»): скрещённые клинки из
  // politics.js читались как рядовая война — а Длань это не война, это ядерка.
  // Шляпка гриба + ножка + линия грунта: четыре штриха, узнаётся мгновенно.
  doom:    '<path d="M4.5 10a8 8 0 0 1 15 0z"/><path d="M9.8 10c.3 4-.7 7-2 10.5"/><path d="M14.2 10c-.3 4 .7 7 2 10.5"/><path d="M3.5 20.5h17"/>',
...
const EST_SV = {
  // Та же кирка крупно (та же беда была и здесь): голова — дуга поперёк
  // черенка, середина дуги лежит РОВНО на его оси (все точки на прямой x+y=130),
  // поэтому черенок входит в проушину, а не обрывается рядом.
  pick: `<g class="pol-sv-tilt"><path d="M30 100 L107 23"/><path d="M72 10 Q114.4 15.6 120 58"/></g>
    <circle class="pol-sv-spark" cx="26" cy="104" r="6"/>`,
