...
const GD_SAY = {
  ...
  spore: ['Спора мира.', 'Такое находят раз в жизни. У меня — второй.',
    'Это не продают. Это только находят.', 'Руки трясутся. Ладно.',
    'Храм, если ты слушаешь, — спасибо.'],
  ore: ['Порода. Тоже хлеб.', 'На счёт державы, не в грядку.', 'Сгодится.',
    'Камень как камень. Взвесим.', 'Не росток, но и не пусто.'],
  dust: ['Пыль.', 'Пустая порода.', 'Всё тот же щебень.', 'Ни зерна.',
    'Ну хоть размялся.'],
  ...
  sow: ['Расти.', 'Тут тебе будет светло.', 'Земли нет, но обойдёмся.',
    ...
    'Корни в железо. Привыкай.', 'Сажаю — и всё ещё не верю.'],
  ...
  feed: ['Раствор пошёл.', 'Ешь.', 'Дорого. Но ты стоишь.', 'Не жадничаю.',
  ...
  harvestFine: ['Как по учебнику.', 'Вот за это и держусь.', 'Полный сбор.'],
  till: ['Стапель развёрнут.', 'Ещё одна ячейка моя.', 'Место есть — будет и посев.',
  ...
  far1: ['Далековато я забрёл.', 'Отсюда кольцо — как обруч на бочке.',
    'Тут уже холодно.', 'Светило за спиной. Непривычно.',
  ...
  idle: ['Надо бы налёт снять. Потом.', 'Лампы гудят.', 'Ещё один день.',
    'Кто-то же должен это делать.', 'Спина.', 'Камни идут густо сегодня.',
    'Дома бы уже чай остыл.', 'Считаю обороты. Со скуки.',
...
function gardenEdgeAsk() {
  ...
  el.innerHTML = `<div class="gd-edge-w">
      <div class="gd-edge-t">кромка</div>
      <p>Дальше светило уже не греет, а сеть кидать не во что. Там просто
      нет ничего — и оно не кончается.</p>
      <p class="gd-edge-q">Держать курс?</p>
      <div class="gd-edge-b">
        <button class="gd-b gd-prim" type="button"
          onclick="event.stopPropagation();gardenEdgeBack()">Повернуть к светилу</button>
        <button class="gd-b gd-ghost" type="button"
          onclick="event.stopPropagation();gardenEdgeGo()">Не поворачивать</button>
      </div></div>`;
...
const GD_EDGE_GO = ['Ну и лечу.', 'Никто не считает мои обороты.',
  'Оттуда видно всё сразу, говорят.', 'А грядки подождут. Они умеют.',
...
async function gardenEdgeGo() {
  ...
  gardenToast('Топливо кончилось раньше пустоты. Ты вернулся.', '');
  gardenPaintOverview();
}
