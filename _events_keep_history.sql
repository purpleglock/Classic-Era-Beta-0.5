-- ============================================================
-- ИСТОРИЯ СОБЫТИЙ НЕ УДАЛЯЕТСЯ
-- Переопределяет _post_covert_rumor и _post_sector_news БЕЗ авто-чистки —
-- старые слухи и сводки больше не пропадают, их можно листать в «Ленте сектора».
-- Выполнить в Supabase → SQL Editor один раз. Идемпотентно.
-- ============================================================

-- Слухи о тайных операциях (без удаления старых)
...
begin
  ...
  insert into public.faction_news(faction_id, faction_name, faction_color, owner_id, owner_email,
    ...
    values (null, '⚠ СЕКТОРНЫЕ СЛУХИ', 'rgba(150,160,180,0.55)', null, null,
      t, null, b, 'approved', 'rumor', now(), now(), now());
  -- без удаления: история сохраняется
end$$;
revoke all on function public._post_covert_rumor(text,text) from public;

-- Сводки сектора (без удаления старых)
