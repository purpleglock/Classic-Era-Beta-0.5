...
async function loadGalaxyData() {
  ...
  try {
    ...
    GM.mzaShips = Array.isArray(mzaShips) ? mzaShips : [];  // мои факельщики — мобильные «Длани» (idle/в полёте)
...
// МАРШ: режим «Звёздный марш» — армии-гарнизоны на колониях, клик = командование.
function gmToggleArmies() {
  GM.showArmies = !GM.showArmies;
  document.getElementById('gm-ctl-armies')?.classList.toggle('gm-active', GM.showArmies);
...
function gmPhantomSeeds() {
  ...
  for (let gx = 0; gx < cols; gx++) {
    for (let gy = 0; gy < rows; gy++) {
      ...
      // Фантом не ставим не только вплотную к звезде, но и внутрь её кольца seed'ов:
      // иначе вес (гигант/кратность) раздувал кольцо, а фантом резал ячейку изнутри —
      // территория так и не росла.
      if (GM.systems.some(s => Math.hypot(s.x - px, s.y - py) < Math.max(MIN_D, gmSysWeight(s) + 70))) continue;
...
function gmSysWeight(s) {
  ...
  // КРАТНОСТЬ = ВЕС. Территория системы — это её ячейка региона, и она была равна
  // ячейке одиночки: фантомные seed'ы (сетка 310) режут её на ~110-155 юнитов. Всё,
  // что глубокий зум рисовал шире, вылезало за собственную границу. Кратной системе
  // физически принадлежит больше — звёзды-компаньоны разнесены на десятки и тысячи
  // а.е., — поэтому её кольцо seed'ов раздвигает соседей и фантомов, и ячейка растёт
  // ЧЕСТНО: граница расширяется вместе с содержимым, а не пробивается им.
  const cs = gmCompStars(s);
...
function gmBuildGeo() {
  ...
  edgeMap.forEach(e => {
    ...
    if (distinct.length >= 2) {
      ...
      facSides.forEach(s => {
        ...
        edges.push({ kind: 'front', fid: s.fid, color: gmSolidColor(fac.color), pts: pts.map(p => [p[0] + ox, p[1] + oy]), nrm: [nx * sign, ny * sign], phase: sign > 0 ? 0 : 0.5 });
      });
      ...
      if (fac.id === 'rift') edges.push({ kind: 'rift', pts });
        ...
        edges.push({ kind: 'fac', fid: fac.id, color: gmSolidColor(fac.color), pts, nrm: [nx * sign, ny * sign] });
      }
  ...
  const secEdges = [];   // {secId, color, pts, nrm}
  edgeMap.forEach(e => {
    ...
    // нормаль ВНУТРЬ сектора (звезда-владелец этой стороны лежит внутри) — по ней
    // уходит вглубь ограждение рубежа; см. gmmPaintSecFence
    const dx = e.b[0] - e.a[0], dy = e.b[1] - e.a[1], L = Math.hypot(dx, dy) || 1;
    const nx = -dy / L, ny = dx / L, mx = (e.a[0] + e.b[0]) / 2, my = (e.a[1] + e.b[1]) / 2;
    [s0, s1].forEach((sid, idx) => {
      ...
      if (!sec) return;
      const sd = e.sides[idx];
      const sign = sd && ((sd.sx - mx) * nx + (sd.sy - my) * ny) < 0 ? -1 : 1;
      secEdges.push({ secId: sid, color: gmNeon(sec.color || '#7cc8ff'), pts, nrm: [nx * sign, ny * sign] });
    });
  });
...
function gmOpenPanel(sys) {
  ...
  // мои факельщики (мобильные «Длани»), стоящие (idle) в этой системе
  (GM.mzaShips || []).filter(sh => sh.status === 'idle' && sh.system_id === sys.id).forEach(sh => {
    defRows.push(`<div class="gm-col-row"><span class="gm-col-dot" style="background:rgba(225,70,55,.9)"></span><span class="gm-col-nm">☣ Факельщик${sh.name ? ': ' + esc(sh.name) : ''}</span><span class="gm-col-ty">${sh.can_fire ? 'готов к залпу' : (sh.in_flight ? 'снаряд в полёте' : 'корпус ' + Math.round(+sh.integrity || 0) + '%')}</span></div>`);
  });
...
// ── Командная плашка факельщика (мобильной «Длани») — клик по ней на карте ──
...
function gmOpenMzaCmd(id) {
  ...
  el.innerHTML = `<div class="gm-opcmd-card">
      ...
      <div class="gm-opcmd-title">☣ Факельщик${sh.name ? ' «' + esc(sh.name) + '»' : ''}</div>
      ...
      <button class="gm-opcmd-btn gm-opcmd-danger" onclick="gmMzaCmdScrap()">✕ Списать факельщика</button>
    </div>`;
  el.classList.remove('gm-hidden');
}
...
function gmMzaCmdSend() {
  ...
  const el = document.getElementById('gm-opcmd');
  if (el) el.innerHTML = `<div class="gm-opcmd-card">
      <div class="gm-opcmd-title">➤ Куда перебросить факельщика?</div>
...
async function gmMzaSendTo(id, destSys) {
  ...
  try {
    ...
    toast('☣ Факельщик в пути · долёт ~' + ((r && r.fly_h) || '?') + ' ч', 'ok');
    gmCloseMzaCmd();
    await gmReloadDefense();
  } catch (e) { toast('Ошибка: ' + (e.message || e), 'err'); gmCloseMzaCmd(); }
  finally { GM._defBusy = false; }
}
// Дальность залпа Факельщика: ПРЫЖКИ по гиперпутям (зеркало _mza_hops в
...
// ── ЗОНА ПОРАЖЕНИЯ Факельщика (режим fireTarget) — сдержанно ──
...
async function gmMzaCmdScrap() {
  ...
  if (!confirm('Списать факельщика? Вернётся ~50% стоимости ГС.')) return;
  if (GM._defBusy) return; GM._defBusy = true;
  try {
    const r = await gmDefRpc('mza_scrap', { p_id: id });
    toast('Факельщик списан · +' + ((r && r.refund) || 0) + ' ГС', 'ok');
    gmCloseMzaCmd();
    await gmReloadDefense();
  } catch (e) { toast('Ошибка: ' + (e.message || e), 'err'); }
  finally { GM._defBusy = false; }
...
//  (носители аванпостов, факельщики, флоты). Клик по строке =
...
function gmRosterUnits() {
  ...
  (GM.mzaShips || []).forEach(sh => out.push({
    type: 'mza', id: sh.id, icon: '☣', name: sh.name || 'Факельщик',
    status: sh.status, sysId: sh.system_id, destId: sh.dest_sys, arrive: sh.arrive_at,
  ...
  (GM.armies || []).forEach(a => out.push({   // МАРШ: армии-гарнизоны
    ...
    status: a.status, sysId: a.system_id, destId: a.dest_system_id, arrive: a.arrive_at,
    extra: (+a.units || 0) + ' юнит.', ready: false, sysName,
  }));
  return out;
}
function gmRosterRender() {
  ...
  let body;
  if (!units.length) {
    body = `<div class="gm-roster-empty">Нет мобильных юнитов.<br>Носители, флоты и факельщик строятся в кабинете на Верфи.</div>`;
  } else {
    const TLAB = { outpost: 'Носители аванпостов', mza: 'Факельщики', fleet: 'Флоты', army: 'Армии (Звёздный марш)' };
    const order = ['fleet', 'army', 'mza', 'outpost'];
...
function gmRosterGo(type, id) {
  ...
  else if (type === 'army') { if (!GM.showArmies) gmToggleArmies(); gmOpenArmyCmd(id); }  // МАРШ
}
...
// ════════════════════════════════════════════════════════════
//  МАРШ: АРМИЯ — командование на карте (режим «Звёздный марш»)
//  Переброска ТОЛЬКО между своими колониями — список кнопками, без прицеливания.
// ════════════════════════════════════════════════════════════
function gmMyColonies() {
  return (GM.colonies || []).filter(c => c.faction_id && GM.myFid && c.faction_id === GM.myFid);
}
function gmOpenArmyStack(ids) {
  const list = (ids || []).map(id => (GM.armies || []).find(x => x.id === id && x.status === 'idle')).filter(Boolean);
  if (!list.length) return;
  if (list.length === 1) { gmOpenArmyCmd(list[0].id); return; }
  GMM.opCmd = null; GMM.mzaCmd = null; GMM.fleetCmd = null;
  GMM.dirty = true; gmmKick();
  const el = document.getElementById('gm-opcmd'); if (!el) return;
  const rows = list.map(a => `<button class="gm-opcmd-btn" onclick="gmOpenArmyCmd('${a.id}')">${esc(a.name || 'Армия')} · ${+a.units || 0} юнит.</button>`).join('');
  el.innerHTML = `<div class="gm-opcmd-card">
      <button class="gm-close" onclick="gmCloseArmyCmd()">✕</button>
      <div class="gm-opcmd-title">Армии на ${esc(list[0].planet_name || 'колонии')}</div>
      <div class="gm-opcmd-sub">${list.length} — выберите</div>
      ${rows}
    </div>`;
  el.classList.remove('gm-hidden');
}
...
function gmCloseArmyCmd() {
  GMM.armyCmd = null; GMM.dirty = true; gmmKick();
  document.getElementById('gm-opcmd')?.classList.add('gm-hidden');
}
async function gmArmySendTo(id, destColony) {
  if (GM._defBusy) return; GM._defBusy = true;
  try {
...
async function gmArmyRecall(id) {
  if (GM._defBusy) return; GM._defBusy = true;
  try {
    const r = await gmDefRpc('army_recall', { p_id: id });
    toast('Армия возвращается домой · ~' + ((r && r.fly_h) || '?') + ' ч', 'ok');
    gmCloseArmyCmd();
    await gmReloadDefense();
  } catch (e) { toast('Ошибка: ' + (e.message || e), 'err'); }
  finally { GM._defBusy = false; }
...
async function gmArmyDisband(id) {
  ...
  try {
    ...
    toast('Армия распущена · +' + ((r && r.returned) || 0) + ' юнит. в состав', 'ok');
    gmCloseArmyCmd();
    await gmReloadDefense();
  } catch (e) { toast('Ошибка: ' + (e.message || e), 'err'); }
  finally { GM._defBusy = false; }
...
function gmmBindCanvas() {
  ...
  const up = (e) => {
    ...
    GMM.gesture = null;
    // Досыл ОДНОГО кадра уже без жеста: гарантирует перерисовку и проверку gmmNeedRaster
    // → чёткую перепечку раст-кэша (вектор/флаги), если во время пана его тянули/сдвигали
    // за пределы покрытия. При инерции это сделает сам gmmFrame на кадре гашения скорости.
    GMM.dirty = true; gmmKick();
...
function gmmTapAt(lx, ly) {
  ...
  // 0б) РЕЖИМ ПРИЦЕЛИВАНИЯ факельщика: отправка / выбор системы-цели для залпа.
  ...
  if (GMM.armyCmd) gmCloseArmyCmd();   // МАРШ
...
function gmmBlit() {
  ...
  // Векторный мир (территории/границы/флаги/пути/сектора) теперь ЗАПЕЧЁН в битмап выше
  // (gmmPaint) и двигается/тянется вместе с ним — живьём его больше не рисуем. Это и
  // убрало основной per-frame расход при пане/зуме (раньше вся геометрия галактики
  // перерисовывалась каждый кадр). См. раст-кэш в gmmPaint.
  ...
  // ◈ ПРЕСТОЛ: якоря — ПОВЕРХ звёзд. Отметка обязана ГАСИТЬ звезду: система,
  // где он доел миры, должна выглядеть дырой в карте, а не звездой с ободком.
  // Под звёздами вырез не работает — светило рисуется сверху и всё сводит на нет.
  gmmPaintAnchors(ctx, GMM.s);
...
// ── ГРАНИЦА СИСТЕМЫ = ЯЧЕЙКА РЕГИОНА, а не окружность ────────────────────────
// Ячейку и видно на карте (тёмный многоугольник владения). Раскладка, упёртая в
// круг «доля пролёта до соседа», неизбежно вылезала за неё: до соседа может быть
// далеко, а грань ячейки — рядом. Здесь считаем, СКОЛЬКО реально места есть в
// нужную сторону, и раскладка упирается в саму границу.
function gmmCellPolys(sys) {
  const c = (GMM.paths && GMM.paths.sysCells) ? GMM.paths.sysCells.get(sys.id) : null;
  return (c && c.polys && c.polys.length) ? c.polys : null;
}
// Дистанция (мировые юниты) от звезды до ВНЕШНЕЙ границы ячейки по направлению ang.
// Ячейка системы — это ОБЪЕДИНЕНИЕ кусков (звезда с весом разворачивается в кольцо
// seed'ов, кусков несколько), поэтому берём ПОСЛЕДНЕЕ пересечение луча: первые —
// это внутренние грани между своими же кусками, они не стены.
// Нет ячейки → Infinity (упор останется прежним, по соседу).
function gmmCellHit(sys, ang) {
  const polys = gmmCellPolys(sys);
  if (!polys) return Infinity;
  const ox = sys.x, oy = sys.y, dx = Math.cos(ang), dy = Math.sin(ang);
  let best = 0;
  for (const pts of polys) {
    for (let i = 0, n = pts.length; i < n; i++) {
      ...
      if (Math.abs(den) < 1e-9) continue;              // луч параллелен грани
      const t = ((a[0] - ox) * ey - (a[1] - oy) * ex) / den;   // вдоль луча
      const u = ((a[0] - ox) * dy - (a[1] - oy) * dx) / den;   // вдоль грани
      if (t > 0 && u >= 0 && u <= 1 && t > best) best = t;
    }
  }
  return best > 0 ? best : Infinity;
}
// Вписанный радиус = самое узкое место ячейки: минимум внешней границы по кругу
// направлений. Круглые части системы (орбиты главной, дымка домена) обязаны
// укладываться в него, иначе вылезут за границу владения.
function gmmCellInradius(sys) {
  if (sys._inr != null) return sys._inr;
  if (!gmmCellPolys(sys)) return (sys._inr = Infinity);
  const N = 48;
  let best = Infinity;
  for (let i = 0; i < N; i++) {
    const h = gmmCellHit(sys, (i / N) * 6.2832);
    if (h < best) best = h;
  }
  ...
  GM.systems.forEach(sys => {
    ...
    const layoutComps = () => {
      ...
      CG.forEach((g, i) => {
        ...
        const share = (g.rHi - rLo) / (CG.length + 0.6) * 0.46;
        ...
        // компаньон стоит у ГРАНИЦЫ своим КРАЕМ, а не центром
        const cLo = rLo + g.miniR + cgap, cHi = Math.max(cLo, g.rHi - g.miniR);
        ...
        placed.push(g);
        // перебор считаем ОТНОСИТЕЛЬНО своего упора: у каждого компаньона он свой
        over = Math.max(over, (r + g.miniR) / Math.max(1, g.rHi));
      });
      return over;
    };
    for (let pass = 0; pass < 8 && CG.length; pass++) {
      const over = layoutComps();
      if (over <= 1) break;
      msc *= (1 / over) * 0.94;   // тесно — ужимаем мини-домены, зазоры сохраняются
    }
...
function gmmRaster(quality) {
  ...
  // окно запекания — чтобы дорогие слои (ограждения рубежей) не считали геометрию
  // всей галактики, когда в кадр попадает её кусок
  GMM.bakeBox = { wx0, wy0, wx1, wy1 };
...
function gmmBuildWorld() {
  ...
  geo.fills.forEach(f => {
    ...
    if (!f.isRift && f.sys) {
      ...
      if (!e) sysCellD.set(f.sys.id, (e = { d: '', reach: 0, polys: [] }));
      e.d += d;
      // polys — сами контуры ячейки в МИРОВЫХ координатах. Нужны глубокому зуму:
      // граница системы — это ячейка региона (её и видно), а НЕ окружность. Раскладка,
      // упёртая в окружность по соседу, вылезала за ячейку там, где та подходит близко.
      e.polys.push(f.pts);
  ...
  //    Тихая граница и линия фронта получают разный мотив, поэтому и цепочки разные:
  //    сшивать их вместе нельзя — иначе зубцы фронта поползли бы по мирному рубежу.
  const facFenceD = new Map();
  geo.edges.forEach(e => {
    if (!e.fid || !e.nrm || (e.kind !== 'fac' && e.kind !== 'front')) return;
    let g = facFenceD.get(e.fid);
    if (!g) facFenceD.set(e.fid, g = { color: e.color, fac: [], front: [] });
    g[e.kind === 'front' ? 'front' : 'fac'].push({ pts: e.pts, nx: e.nrm[0], ny: e.nrm[1] });
  });
  let lanesD = '';
  ...
  // ячейки пересобраны → вписанный радиус устарел (звезду могли подвинуть в редакторе)
  GM.systems.forEach(s => { s._inr = null; });
  // границы секторов, сгруппированные по цвету (для пунктира)
  const secD = new Map();
  (geo.secEdges || []).forEach(e => secD.set(e.color, (secD.get(e.color) || '') + dOf(e.pts)));
  // ── ОГРАЖДЕНИЕ СЕКТОРОВ: по КАЖДОМУ сектору — сырые отрезки рубежа с нормалью
  //    внутрь. Отрезки нужны точками, а не готовым Path2D: рельсы и узлы кладутся
  //    поштучно со смещением вглубь. Группировка по цвету (secD) не годится —
  //    сектора одного цвета замаскировали бы ограждение друг друга.
  const secBrushD = new Map();
  (geo.secEdges || []).forEach(e => {
    if (!e.secId || !e.nrm) return;
    let b = secBrushD.get(e.secId);
    if (!b) secBrushD.set(e.secId, (b = { color: e.color, strokes: [] }));
    b.strokes.push({ pts: e.pts, nx: e.nrm[0], ny: e.nrm[1] });
  });
  GMM.paths = {
    ...
    sysCells: new Map([...sysCellD].map(([id, e]) => [id, { p2d: new Path2D(e.d), reach: e.reach, polys: e.polys }])),
    ...
    secBrush: [...secBrushD].map(([secId, b]) => {
      const area = secHitD.get(secId);
      if (!area) return null;
      const chains = gmmChainSecEdges(b.strokes);
      return chains.length ? { secId, color: b.color, chains, area: new Path2D(area) } : null;
    }).filter(Boolean),
    facFence: [...facFenceD].map(([fid, g]) => {
      const area = facMaskD.get(fid);
      if (!area) return null;
      const fac = gmmChainSecEdges(g.fac), front = gmmChainSecEdges(g.front);
      return (fac.length || front.length)
        ? { fid, color: g.color, fac, front, area: new Path2D(area) } : null;
    }).filter(Boolean),
...
function gmmBuildDefense() {
  ...
  if ((GM.opShips || []).length || (GM.mzaShips || []).length || (GM.fleets || []).length
    ...
    // `extra` навешивает спец-поля (для факельщика: mza/canFire/integrity).
    ...
    // Факельщики — мобильные «Длани»: красный отблеск + флаг возможности залпа.
    ...
    // Вскрытые чужие факельщики (только idle — позиция точная; transit без пути не рисуем).
...
function gmmPaintDefense(ctx) {
  ...
  {
    const grp = {};
    // группируем по СТОРОНЕ (left=флоты, right=носители/факельщики) и по ВЛАДЕЛЬЦУ
  ...
  D.ships.forEach(d => {
    try {
      ...
      if (d.kind === 'idle') {
        ...
        // римской цифрой. Носители/факельщики по-прежнему раскладываем веером.
        ...
        // флоты сидят СЛЕВА от звезды (side='left'), носители/факельщики — справа.
        ...
        // Чужой вскрытый факельщик — тревожный пульс-кольцо «обнаружен»
        if (d.mza && d.enemy) {
        ...
        // Факельщик: шкала корпуса НАД флагом (виден износ прямо на карте) — только свой
        if (d.mza && !d.enemy) {
...
function gmmPaint(ctx, camS, wx0, wy0, wx1, wy1) {
  ...
  // РАСТ-КЭШ: векторный мир (территории/границы/флаги/гиперпути/сектора) ЗАПЕКАЕМ в
  // битмап. Раньше он рисовался живьём КАЖДЫЙ кадр — при пане/зуме это перерисовывало
  // всю геометрию галактики (заливки + штрих границ ×3 + флаги) 60 раз в секунду и
  // роняло FPS. Теперь тяжёлый слой печётся ОДИН раз (на паузе жеста, debounce 90мс) и
  // при движении просто двигается/тянется как единое изображение — ничего не пропадает.
  // Плата: при ЗУМЕ картинка на миг чуть размывается (битмап тянется) и становится
  // чёткой, когда жест уймётся и битмап перепечётся (как в Google Maps). При ПАНЕ
...
function gmmFlagLayer(f) {
  ...
  // видно много флагов сразу). Ступени = степени двойки (128..1024), чтобы не
  // пересобирать слой на каждый пиксель зума. Уже собранный слой с бОльшим/равным
  // разрешением переиспользуем (лишний даунскейл дёшев); пересобираем ТОЛЬКО когда
  // нужно чётче — при заходе вглубь. Пересборка = максимум ~3 раза на весь диапазон.
  const need = Math.max(f.bw, f.bh) * GMM.s * GMM.dpr;   // длинная сторона в экранных px
  const want = Math.min(1024, Math.max(128, Math.pow(2, Math.ceil(Math.log2(Math.max(1, need))))));
  let c = cache[f.fid];
  if (c && c._res >= want) return c;
  const aspect = f.bw / f.bh, MAX = want;
  let W, H;
...
function gmmPaintVector(ctx, camS, live) {
  ...
  if (secShow && P.secFills && P.secFills.length && secA > 0.01) {
    ctx.save();
    // Заливка приглушена: плотное тело .5 + экранная подсветка .3 сверху давали почти
    // непрозрачный цветной ковёр — он забивал и территории фракций, и звёзды, и всю
    // разметку рубежей. Сектор должен читаться ПОДЛОЖКОЙ под картой, а не поверх неё.
    ctx.globalAlpha = .26 * secA;
    P.secFills.forEach(f => { ctx.fillStyle = f.color; ctx.fill(f.p2d); });
    ctx.globalCompositeOperation = 'screen'; ctx.globalAlpha = .12 * secA;
    P.secFills.forEach(f => { ctx.fillStyle = f.color; ctx.fill(f.p2d); });
    ctx.restore();
  }
  if (GM.showBorders) {
    ...
    // вблизи уезжала вглубь территории. Только в запекаемый битмап — clip по площади
    // и тысячи насечек не потянуть в живом слое каждый кадр.
    if (!live) gmmPaintFacFence(ctx, camS, facB);
  ...
  if (secShow && P.secEdges && P.secEdges.length && secA > 0.01) {
    ...
    // это ровно тот же мотив «перпендикулярная риска через равный шаг», что и стойки
    // эмиттеров. Два таких слоя друг на друге читались как грязь вдоль линии.
    ctx.globalAlpha = .9 * secA; ctx.lineWidth = 1.3 / camS;
    P.secEdges.forEach(e => { ctx.strokeStyle = e.color; ctx.stroke(e.p2d); });
    ctx.globalAlpha = 1;
    // Ограждение — ПОВЕРХ ядра и ТОЛЬКО в запекаемый битмап: clip по площади сектора
    // плюс тысячи сегментов рельсов слишком дороги для живого слоя каждого кадра
    // (та же сделка, что у тумана и звёздной россыпи — см. шапку gmmPaintVector).
    if (!live) gmmPaintSecFence(ctx, camS, secA);
  }
...
// Сшивание рубежа в НЕПРЕРЫВНЫЕ контуры. Вороное отдаёт границу россыпью независимых
// рёбер; мазок по каждому ребру отдельно и давал «лесенку» — штрих обрывался на каждой
// вершине, а band выглядел рубленым. Кисть идёт ОДНИМ ДВИЖЕНИЕМ через десяток рёбер,
// поэтому сначала собираем цепочки по совпадающим концам. Нормаль «внутрь» тащим с
// собой поточечно: на изгибе контура она разворачивается вместе с ним.
function gmmChainSecEdges(strokes) {
  const KEY = p => Math.round(p[0] * 2) + ',' + Math.round(p[1] * 2);
  const ends = new Map();                    // ключ узла → [индексы отрезков]
  const at = k => { let a = ends.get(k); if (!a) ends.set(k, a = []); return a; };
  strokes.forEach((st, i) => {
    at(KEY(st.pts[0])).push({ i, head: true });
    at(KEY(st.pts[st.pts.length - 1])).push({ i, head: false });
  });
  const used = new Array(strokes.length).fill(false);
  const free = key => { for (const e of (ends.get(key) || [])) if (!used[e.i]) return e; return null; };
  const chains = [];
  for (let i = 0; i < strokes.length; i++) {
    if (used[i]) continue;
    used[i] = true;
    const s0 = strokes[i];
    // Нормаль живёт на СЕГМЕНТЕ (nrm[k] — между pts[k] и pts[k+1]), а не на точке.
    // На точке она неизбежно смещается на единицу при доращивании цепочки, и мазок
    // на изгибе уезжал бы по нормали СОСЕДНЕГО ребра — то есть вбок от рубежа.
    let pts = s0.pts.map(p => [p[0], p[1]]);
    let nrm = new Array(Math.max(0, pts.length - 1)).fill(0).map(() => [s0.nx, s0.ny]);
    for (let g = 0; g < 500; g++) {          // наращиваем ВПЕРЁД от хвоста
      const e = free(KEY(pts[pts.length - 1]));
      if (!e) break;
      used[e.i] = true;
      const st = strokes[e.i], seq = e.head ? st.pts : st.pts.slice().reverse();
      for (let k = 1; k < seq.length; k++) { pts.push([seq[k][0], seq[k][1]]); nrm.push([st.nx, st.ny]); }
    }
    for (let g = 0; g < 500; g++) {          // и НАЗАД от головы
      const e = free(KEY(pts[0]));
      if (!e) break;
      used[e.i] = true;
      const st = strokes[e.i];
      // берём отрезок так, чтобы он ЗАКАНЧИВАЛСЯ в текущей голове цепочки
      const seq = e.head ? st.pts.slice().reverse() : st.pts;
      const add = [], addN = [];
      for (let k = 0; k < seq.length - 1; k++) { add.push([seq[k][0], seq[k][1]]); addN.push([st.nx, st.ny]); }
      pts = add.concat(pts); nrm = addN.concat(nrm);
    }
    const cum = [0];
    let x0 = 1e9, y0 = 1e9, x1 = -1e9, y1 = -1e9;
    for (let k = 0; k < pts.length; k++) {
      if (k) cum.push(cum[k - 1] + Math.hypot(pts[k][0] - pts[k - 1][0], pts[k][1] - pts[k - 1][1]));
      const p = pts[k];
      if (p[0] < x0) x0 = p[0]; if (p[0] > x1) x1 = p[0];
      if (p[1] < y0) y0 = p[1]; if (p[1] > y1) y1 = p[1];
    }
...
function gmmPaintStarfield(ctx, camS, wx0, wy0, wx1, wy1) {
  ...
  for (let li = 0; li < layers.length; li++) {
    ...
    for (let i = i0; i <= i1; i++) for (let j = j0; j <= j1; j++) for (let k = 0; k < 2; k++) {
      ...
      // РАЗМЕР: пыль, а не звёзды. Было 0.32..0.82px радиуса — на обзоре, когда вся
      // галактика в кадре и россыпь покрывает экран целиком, это читалось как крупный
      // белый крап поверх карты. Фон обязан быть фоном.
      const r = (0.20 + br * br * 0.34) * u;
